...
import { Accordion, AccordionItem, Button, Image, Input, Listbox, ListboxItem, Textarea } from '@nextui-org/react'
...
function AdminInstructorView() {
    ...
    const handleActive = () =>{
        setactivebtn("loading")
        axiosInstance.get(`/eduadmin/instructor_status/${instructor_id}`)
        .then(response=>{     
        ...
        <div className='grid grid-cols-3 grid-flow-row-dense mt-4'>
              <div className='mx-16'> 
                <Image
                    ...
                    src={instructor.image?`${API_URL}/${instructor.image}`:`${STATIC_IMAGE_URL}/profileicon.jpg`}
             ...
             <div className='col-span-2 gap-4 grid grid-cols-2 '>
               ...
               {activebtn == true && <Button onClick={handleActive} color='success'>Active</Button> }
                ...
                {activebtn == 'loading' && <Button isLoading color="secondary"  spinner={<svg
